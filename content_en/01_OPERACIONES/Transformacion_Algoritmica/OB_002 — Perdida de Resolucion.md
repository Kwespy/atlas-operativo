---
title: "OB_002 — Loss of Resolution"
atlas_direct: true
lang: en
---


# OB_002 — Loss_Of_Resolution_Compression

## Status
Status:: [[Terminada|Finished]]


## 1. Operation data

**Internal number:** OB_002  
**Operational regime:** [[Lista_Transformacion_Algoritmica|Algorithmic Transformation List]]  
**Source image:** Photograph taken from life

**Operational description:**  
Through a script that gradually compresses the file, the image lost resolution until it became pixelated.

---

## 2. Tools and procedure

**Tools / medium:**  
- Macbook Pro 2018
- Terminal App / Script
- ChatGPT / Code

**Script (2026-09 audit):** Lost (not recovered in 2026-09 audit) — the file indicates that a script was created and executed in Terminal with the help of ChatGPT/Code, but the file was not saved in the Atlas.

**Technical procedure:**  
1. Script was created 

---

## 3. Variables

**Controlled variables:**  
- Number of steps that can be performed
-  
- 

**Uncontrolled / accidental variables:**  
- Number of pixels
- Image size 
- 

---

## 4. Visual sequence

### Initial image
![[OB002_input.jpg|200]]

### Step 01
![[OB002_step01.jpg.webp|200]]

### Step 02
![[OB002_step02.jpg.webp|200]]

### Step 03
![[OB002_step03.jpg.webp|200]]

### Step 04
![[OB002_step04.jpg.webp|200]]

### Step 05
![[OB002_step05.jpg.webp|200]]

### Step 06
![[OB002_step06.jpg.webp|200]]

### Step 07
![[OB002_step07.jpg.webp|200]]

### Step 08
![[OB002_step08.jpg.webp|200]]

### Step 09
![[OB002_step09.jpg.webp|200]]

### Step 10
![[OB002_step10.jpg.webp|200]]

### Step 11
![[OB002_step11.jpg.webp|200]]

### Step 12
![[OB002_step12.jpg.webp|200]]


---

## 5. Visual selection

### Crisis / threshold according to AI-assisted reading / ChatGPT

![[OB002_step06.jpg.webp|200]]
### Crisis / threshold according to my perception
![[OB002_step05.jpg.webp|200]]


---

## 6. Crisis and formal-perceptual reading

**Main crisis:** resolution - technique

The operation affects the visual grammar of the image: shape, contour, color, figure/ground relationship, legibility, recognition, and support.

**Shape / contour / figure-ground:**  
The image loses its shape rapidly, but the background is not lost until the image is completely destroyed.

**Color / tone:**  
The colors remain mostly stable.

**Legibility / recognition:**  
Legibility is what is most affected; recognition is lost due to the lack of contours.

**Material / support:**  
The materiality and support are not visible, however, it is easily noticeable as a digital aesthetic or process.

**Perceptual effect:**  
If you squint your eyes, it is easier to recognize the object.

---

## 7. Final evaluation and artistic reference

**Does this operation truly put the image into crisis, or does it function merely as an aesthetic effect?**  
It puts the image into crisis

**Operation notes / what I learned:**  
It works, but it is an operation that can be understood as mannerism. What surprised me was that the AI was able to recognize objects in low resolution better than I could. It puts the image into crisis while opening up pictorial possibilities at first glance.

**Final_status:** 
Works:: [[SI|YES]]

**How it destroys the image:** 
Works_on:: [[Formal|Formal]] 

**Level and richness of crisis:** 
Crisis::  [[Media|Medium]]

**Decision:** 
Selection:: [[Combinable|Combinable]] / [[Guardar|Save]] 


---

### Possible artistic reference

Reference:: [[Thomas Ruff|Thomas Ruff]] [[Rosa Menkman|Rosa Menkman]]

**Possible works:**  
- Thomas Ruff — _jpeg_  
- Rosa Menkman -- https://www.buzdokuz.com/wp-content/uploads/2022/06/1-Rosa-Menkman-A-Vernacular-of-File-Formats_compressed.pdf


---

### Possible artistic reference

**Reference:**  
Thomas Ruff — _jpeg_  
Cory Arcangel 
Rosa Menkman — glitch/compression artifacts
Andy Denzler
