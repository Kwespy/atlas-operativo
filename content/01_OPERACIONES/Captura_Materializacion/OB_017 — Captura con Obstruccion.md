---
title: "OB_017 — Captura con Obstruccion"
atlas_direct: true
---

# OB_017_Captura_con_Obstruccion

## Estado
Estado::  [[Terminada]]

---

## 1. Datos de la operación

**Número interno:** OB_017 
**Régimen operativo:** [[Lista_Captura_Materializacion]]  
**Imagen de origen:**  Fotografía

**Descripción operativa:**  
Delante de la camara del movil se pone un vaso con agua para que actue como filtro y obstaculo. se va llenando el vaso con agua gradualmente.

---

## 2. Herramientas y procedimiento

**Herramientas / medio:**  
- Camara de foto
- Vaso
- Agua

**Procedimiento técnico:**  
1. Se va llenando el vaso con agua gradualmente y se le va sacando fotos.
2. 
3. 
4. 
5. 

---

## 3. Variables

**Variables controladas:**  
- Cantidad de agua
- Perspectiva de la cámara
- Luz ambiente

**Variables no controladas / accidentales:**  
- Reflección del agua y las formas del vaso.
- Como se deforma la imagen
- 

---

## 4. Secuencia visual

### Imagen inicial
![[Input_001.jpg.webp|200]]

### Step 01
![[OB017_step01.jpeg.webp|200]]

### Step 02
![[OB017_step02.jpeg.webp|200]]

### Step 03
![[OB017_step03.jpeg.webp|200]]

### Step 04
![[OB017_step04.jpeg.webp|200]]

### Step 05
![[OB017_step05.jpeg.webp|200]]

### Step 06
![[OB017_step06.jpeg.webp|200]]

### Step 07
![[OB017_step07.jpeg.webp|200]]

### Step 08
![[OB017_step08.jpeg.webp|200]]

### Step 09
![[OB017_step09.jpeg.webp|200]]

### Step 10
![[OB017_step10.jpeg.webp|200]]

### Step 11
![[OB017_step11.jpeg.webp|200]]


---

## 5. Selección visual

### Crisis / umbral según lectura asistida por IA / ChatGPT

### Step 05
![[OB017_step05.jpeg.webp]]

### Crisis / umbral según mi percepción


### Step 07
![[OB017_step07.jpeg.webp]]

---

## 6. Crisis y lectura formal-perceptiva

**Crisis principal:** perdida de reconocimiento por refracció y fragmentación del contorno.

**La operación afecta la gramática visual de la imagen: forma, contorno, color, relación figura/fondo, legibilidad, reconocimiento y soporte:**

el vaso ya es un filtro que transforma la forma y e contorno. los dos juntos mexcla el fondo con la forma y la legibilidad se pierde.  Los colores se mantienen.


---

---

## 7. Evaluación final y referente artístico


**Notas de la operación / qué aprendí:**  
Es una operación simple, y la transformación es de a forma parecidad a lo que puede lograr un algoritmo. es dificil de controlar.


**Estado_final:** 
Funciona:: [[SI]]

**Como destruye la imagen:** 
Trabaja_en_lo:: [[Formal]] 

**Nivel y Riqueza de crisis:** 
Crisis::  [[Alta]]

**Decisión:** 
Seleccion:: [[Combinable]] / [[Guardar]] 

---

### Referente artístico posible

Referente:: [[Daisuke Yokota]], [[Luc Tuymans]]

**Obras posibles:**  
- [Daisuke Yokota](https://www.google.com/search?q=5.+Daisuke+Yokota+paintings&sca_esv=e44b8a29c1fb6f03&rlz=1C5CHFA_enPT1033PT1033&udm=2&biw=949&bih=952&sxsrf=APpeQnvIhGrwYpejtzitH2YoMLjQs8ANrg%3A1783015280104&ei=cKdGatPrBYL06APe16uIBA&ved=0ahUKEwjT45jDybSVAxUCOnoKHd7rCkEQ4dUDCBE&uact=5&oq=5.+Daisuke+Yokota+paintings&gs_lp=Egtnd3Mtd2l6LWltZyIbNS4gRGFpc3VrZSBZb2tvdGEgcGFpbnRpbmdzSIQjUMMEWIQicAR4AJABAJgBaqABoAiqAQM5LjK4AQPIAQD4AQGYAgCgAgCYAwCIBgGSBwCgB4QBsgcAuAcAwgcAyAcAgAgB&sclient=gws-wiz-img)

- [Luc Tuymans](https://aperture.org/editorial/the-photographic-traces-of-luc-tuymanss-ghostly-paintings/)
